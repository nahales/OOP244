...
namespace ict {
	...
	//setter
	/*void Item::set(const char* SKU, const char* NAME, double PRICE, int QUANTITY, bool TAXED)
	{
		sku(SKU);
		price(PRICE);
		name(NAME);
	...
	std::ostream& Item::write(std::ostream& ostrm, bool linear)const
	{
		return ostrm;
	}

	std::istream& Item::read(std::istream& istrm)
	{
		return istrm;
	}
}
